# Summary

- [Introduction to MSALT](README.md)

# Getting Started
- [Installation & Requirements](setup/installation.md)
- [Label Configuration](setup/configuration.md)
- [Sensor Setup and configuration & Calibration](setup/sensors.md)

# User Interface
- [UI Overview & Layout](ui/overview.md)
- [3D LiDAR View](ui/lidar_view.md)
- [2D Camera Strip & Pop-Out Modal](ui/camera_view.md)
- [Inspector & Batch Grid View](ui/inspector_grid.md)

# Annotation Workflows
- [Manual 3D Drawing](workflows/manual_drawing.md)
- [Auto-Annotation (YOLOv8 + SAM2)](workflows/auto_annotation.md)
- [Tracking (Kalman Filter)](workflows/kalman_tracking.md)
- [Interpolation (SAM2 + PCA)](workflows/interpolation.md)
- [Shortcut Keys & Controls](workflows/shortcuts.md)

# Evaluation & Benchmarking
- [Running the NuScenes Evaluator](benchmarking/nuscenes.md)
- [Understanding the Metrics](benchmarking/metrics.md)

# Developer Guide
- [System Architecture](dev/architecture.md)
- [The Data Controller](dev/data_controller.md)
- [Geometry & Raycasting Math](dev/geometry.md)
- [Testing](dev/testing.md)