...
def check_setup():
    ...
    if not models_dir.exists():
        print(f"CRITICAL: Directory 'config/models' does not exist. Found: {list(root.glob('*'))}")
        return

    # Check Default Model File
    model_file = models_dir / "default.yaml"
    if not model_file.exists():
        print(f"CRITICAL: 'config/models/default.yaml' MISSING! Files in models/: {list(models_dir.glob('*'))}")
        return
    ...
    try:
        ...
        if 'segmentation' not in conf:
            print("CRITICAL ERROR: 'segmentation' key missing from default.yaml. Check indentation!")
        else:
