...
class AnalyticsDashboard(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.btn_refresh.setStyleSheet(
            "background-color: #151B54; color: white; font-weight: bold;"
        )
        ...
        self.plt_lifespan = pg.PlotWidget(
            title="Consistency (Track Lifespan in Frames)"
        )
    ...
    def _compute_and_draw(self):
        ...
        for f_idx in range(start, end + 1):
            ...
            for box in boxes:
                ...
                if box.track_id not in id_volumes or volume > id_volumes[box.track_id]:
                    id_volumes[box.track_id] = (
                        volume  # Store max volume seen for this ID
                    )
        ...
        self.plt_completeness.plot(
            frames, counts, pen=pen, fillLevel=0, brush=(0, 255, 0, 50)
        )
