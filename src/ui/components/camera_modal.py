from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QPushButton, QGraphicsTextItem,
...
class DrawableGraphicsView(QGraphicsView):
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self.current_rect_item = QGraphicsRectItem(QRectF(self.start_pt, self.start_pt))
            self.current_rect_item.setPen(QPen(QColor(0, 255, 0), 2)) # Bright Green Box
...
class CameraPopOutModal(QDialog):
    ...
    def __init__(self, pixmap: QPixmap, cam_name: str, existing_boxes=None, parent=None):
        ...
        if existing_boxes:
            for box_data in existing_boxes:
                ...
                text_item.setPos(box_data['x'], box_data['y'] - 30) # Position just above the box
