...
class MiniFrameWidget(gl.GLViewWidget):
    ...
    def _draw_scene(self):
        ...
        line_item = gl.GLLinePlotItem(
            pos=np.array(pts), mode="lines", color=(0, 1, 0, 1), width=2, antialias=True
        )
    ...
    def set_view_mode(self, mode: str):
        ...
        if mode == "TOP":
            ...
            self.setCameraPosition(
                distance=telephoto_dist, elevation=90, azimuth=-90 - h_deg
            )
            ...
            world_y_dir = np.array(
                [math.sin(self.box.heading), math.cos(self.box.heading), 0]
            )
            ...
            self.setCameraPosition(
                distance=telephoto_dist, elevation=0, azimuth=azimuth_deg
            )
            ...
            world_x_dir = np.array(
                [math.cos(self.box.heading), math.sin(self.box.heading), 0]
            )
...
class BatchGridWindow(QWidget):
    ...
    def __init__(self, data_controller, annotation_manager):
        ...
        self.setWindowTitle(
            "Batch Correction (WASD to Move, QE to Rotate, RF to move up down, Shift along with keys for scaling)"
        )
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if changed:
            ...
            self.header.setText(
                f"Editing Frame {self.active_frame_idx} | Box: x={box.x:.3f}, y={box.y:.3f} z={box.z:.3f} | Scale: dx={box.dx:.3f}, dy={box.dy:.3f} dz={box.dz:.3f} | Heading: {box.heading}"
            )
    ...
    def change_all_views(self, mode: str):
        ...
        if mode == "SIDE":
            self.header.setText(
                "Side View (XZ). Controls: WASD move X/Z. (Y-axis hidden)"
            )
        elif mode == "TOP":
            self.header.setText(
                "Top View (XY). Controls: WASD move X/Y. (Z-axis hidden)"
            )
        elif mode == "FRONT":
            self.header.setText(
                "Front View (YZ). Controls: WASD move Z/Y. (X-axis hidden)"
            )
        else:
