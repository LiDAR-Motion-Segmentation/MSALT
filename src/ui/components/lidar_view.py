...
class CustomGLWidget(gl.GLViewWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.ground_z = (
            0  # Default ground plane for first ray cast (can be overridden by config)
        )
    ...
    def _calculate_obb(self):
        ...
        p2 = (
            self.current_mouse_pt if len(self.base_points) == 1 else self.base_points[1]
        )
        if p2 is None:
            p2 = p1
    ...
    def mousePressEvent(self, event):
        ...
        if event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            if event.button() == Qt.MouseButton.LeftButton:
                ...
                origin, direction = self._screen_to_ray_qt(
                    event.pos().x(), event.pos().y()
                )

                if self.state == DrawState.IDLE:
                    hit = GeometryUtils.intersect_ray_plane(
                        origin, direction, self.ground_z
                    )
                    ...
                    hit = GeometryUtils.intersect_ray_plane(
                        origin, direction, self.draw_ground_z
                    )
                    if hit is not None:
    ...
    def mouseMoveEvent(self, event):
        if self.state == DrawState.DRAGGING_BASE:
            plane_z = (
                self.draw_ground_z if self.draw_ground_z is not None else self.ground_z
            )
    ...
    def paintEvent(self, event):
        ...
        if self.state == DrawState.DRAGGING_BASE:
            ...
            painter.drawText(
                10, 24, "Ctrl+Click: Confirm Box | Scroll/Drag: Adjust Height"
            )
        ...
        proj_matrix = get_projection_matrix(
            w, h, self.opts.get("fov", 60), self.opts.get("distance", 20)
        )
...
class LidarVisualizer(BasePluginWidget):
    ...
    def __init__(self, parent=None, cfg=None):
        ...
        self.ground_percentile = (
            getattr(cfg, "ground_percentile", 0.5) if cfg is not None else 0.5
        )
        self.ground_bias = (
            getattr(cfg, "ground_bias", 0.05) if cfg is not None else 0.05
        )
        self.default_ground_z = (
            getattr(cfg, "default_ground_z", -1.5) if cfg is not None else -1.5
        )
        ...
        # Laser Pointer Ray
        self.laser_ray = gl.GLLinePlotItem(
            color=(1.0, 0.0, 0.0, 0.5), width=2, antialias=True
        )
    ...
    def _setup_ui(self):
        ...
        self.chk_occlusion.setStyleSheet(
    ...
    def _draw_points_default(self):
        ...
        colors[:, 1] = 0.5
        self.scatter.setData(pos=self.current_points, color=colors, size=persisted_size)
    ...
    def update_boxes(self, boxes: list[BoundingBox3D]):
        ...
        if self.current_points is not None:
            ...
            persisted_size = getattr(self, "current_point_size", 2)
            self.scatter.setData(pos=points, color=colors, size=persisted_size)
    ...
    def update_laser_pointer(self, origin, direction, hit_point=None):
        if (
            not getattr(self, "occlusion_mode", False)
            or origin is None
